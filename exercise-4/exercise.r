...
movie.data <- fromJSON(request_content)

# Make movie_data into a data frame using as.data.frame
movie_df <- as.data.frame(movie.data)
...
Director <- function(df){
  director <- paste(df$Title, "was directed by", df$Director)
  return(director) 
}

# Call Director with your favorite movie, and assign it to the variable movie.director
movie.dircetor <- Dircetor(movie_df)
